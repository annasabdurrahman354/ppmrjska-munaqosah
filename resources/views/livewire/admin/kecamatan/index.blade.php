<div>
    <div class="card-controls sm:flex">
        <div class="w-full sm:w-1/2">
            Per page:
            <select wire:model="perPage" class="form-select w-full sm:w-1/6">
                @foreach($paginationOptions as $value)
                    <option value="{{ $value }}">{{ $value }}</option>
                @endforeach
            </select>

            @can('kecamatan_delete')
                <button class="btn btn-rose ml-3 disabled:opacity-50 disabled:cursor-not-allowed" type="button" wire:click="confirm('deleteSelected')" wire:loading.attr="disabled" {{ $this->selectedCount ? '' : 'disabled' }}>
                    {{ __('Delete Selected') }}
                </button>
            @endcan

            @if(file_exists(app_path('Http/Livewire/Admin/ExcelExport.php')))
                <livewire:admin.excel-export model="Kecamatan" format="csv" />
                <livewire:admin.excel-export model="Kecamatan" format="xlsx" />
                <livewire:admin.excel-export model="Kecamatan" format="pdf" />
            @endif




        </div>
        <div class="w-full sm:w-1/2 sm:text-right">
            Search:
            <input type="text" wire:model.debounce.300ms="search" class="w-full sm:w-1/3 inline-block" />
        </div>
    </div>
    <div wire:loading.delay>
        Loading...
    </div>

    <div class="overflow-hidden">
        <div class="overflow-x-auto">
            <table class="table table-index w-full">
                <thead>
                    <tr>
                        <th class="w-9">
                        </th>
                        <th class="w-28">
                            {{ trans('cruds.kecamatan.fields.id') }}
                            @include('components.table.sort', ['field' => 'id'])
                        </th>
                        <th>
                            {{ trans('cruds.kecamatan.fields.kabupaten') }}
                            @include('components.table.sort', ['field' => 'kabupaten.name'])
                        </th>
                        <th>
                            {{ trans('cruds.kecamatan.fields.name') }}
                            @include('components.table.sort', ['field' => 'name'])
                        </th>
                        <th>
                        </th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($kecamatans as $kecamatan)
                        <tr>
                            <td>
                                <input type="checkbox" value="{{ $kecamatan->id }}" wire:model="selected">
                            </td>
                            <td>
                                {{ $kecamatan->id }}
                            </td>
                            <td>
                                @if($kecamatan->kabupaten)
                                    <span class="badge badge-relationship">{{ $kecamatan->kabupaten->name ?? '' }}</span>
                                @endif
                            </td>
                            <td>
                                {{ $kecamatan->name }}
                            </td>
                            <td>
                                <div class="flex justify-end">
                                    @can('kecamatan_show')
                                        <a class="btn btn-sm btn-info mr-2" href="{{ route('admin.kecamatans.show', $kecamatan) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan
                                    @can('kecamatan_edit')
                                        <a class="btn btn-sm btn-success mr-2" href="{{ route('admin.kecamatans.edit', $kecamatan) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan
                                    @can('kecamatan_delete')
                                        <button class="btn btn-sm btn-rose mr-2" type="button" wire:click="confirm('delete', {{ $kecamatan->id }})" wire:loading.attr="disabled">
                                            {{ trans('global.delete') }}
                                        </button>
                                    @endcan
                                </div>
                            </td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="10">No entries found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    </div>

    <div class="card-body">
        <div class="pt-3">
            @if($this->selectedCount)
                <p class="text-sm leading-5">
                    <span class="font-medium">
                        {{ $this->selectedCount }}
                    </span>
                    {{ __('Entries selected') }}
                </p>
            @endif
            {{ $kecamatans->links() }}
        </div>
    </div>
</div>

@push('scripts')
    <script>
        Livewire.on('confirm', e => {
    if (!confirm("{{ trans('global.areYouSure') }}")) {
        return
    }
@this[e.callback](...e.argv)
})
    </script>
@endpush